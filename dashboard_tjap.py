...
with tab4:
    ...
    for ano in sorted(df_all["Ano"].unique()):
        ...
        metricas_anuais[ano] = {
            ...
            # Metodologia correta: numerador e denominador filtrados pelo mesmo canal
            "Presencial OK (%)":      round(
                (d["canal"].str.contains("Presencial", na=False) & (d["presencial_ok"]=="Sim")).sum() /
                max(d["canal"].str.contains("Presencial", na=False).sum(), 1) * 100, 1),
            "Balcão Virtual OK (%)":  round(
                (d["canal"].str.contains("Virtual", na=False) & (d["virtual_ok"]=="Sim")).sum() /
                max(d["canal"].str.contains("Virtual", na=False).sum(), 1) * 100, 1),
            "Portal OK (%)":          round(
                (d["canal"].str.contains("Portal/site", na=False) & (d["portal_ok"]=="Sim")).sum() /
